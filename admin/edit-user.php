...
$email=$_POST['email'];
$id=$_GET['id'];
$query="update registration set name=?,email=? where id=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iii',$name,$email,$id);
$stmt->execute();
	...
	$ret="select * from registration where id=?";
	 ...
	 //$cnt=1;
	   while($row=$res->fetch_object())
	  {
		...
		<div class="col-sm-8">
	<input type="text" class="form-control" name="pass" id="rmno" value="<?php echo $row->password;?>" disabled>
	<span class="help-block m-b-none">
													Password can't be changed.</span>
						 </div>
						</div>
<div class="form-group">
